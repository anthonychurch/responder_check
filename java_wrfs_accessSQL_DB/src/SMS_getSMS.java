...
class SMS_getSMS {
    ...
    public SMS_getSMS(String username, String password){
    	m4uUser = username;
    	m4uPass = password;
	}
    ...
    private static void testSendMessages (String testPhone, String testMessage, boolean secureMode, boolean debug, String debugFile) {
        ...
        si.addMessage (testPhone, testMessage, 0, 0,
                    ValidityPeriod.DEFAULT, false);
    ...
    public static String[] getTimeRecieved(SmsReply sr){
		...
		if(when > timeCap){
			...
			String backTime = TimeStamp_Utilities.convertToTimeStamp(when);
			GetCurrentTimeStamp timeStamp = new GetCurrentTimeStamp();
			String tss = TimeStamp_Utilities.getTimeStampSeconds(timeStamp);
			String backDate = TimeStamp_Utilities.getBackDate(tss, backTime);
