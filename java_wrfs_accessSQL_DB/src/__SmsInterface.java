...
public class SmsInterface
{
  ...
  public SmsInterface(int paramInt)
  {
      ...
      if (this._httpProxy != "") {
        ...
        if ((this._proxyUsername != "") && (this._proxyPassword != ""))
          Authenticator.setDefault(new Authenticator()
          {
            ...
            {
              PasswordAuthentication localPasswordAuthentication = new PasswordAuthentication(SmsInterface.this._proxyUsername, SmsInterface.this._proxyPassword.toCharArray());
  ...
  {
	  System.out.println("///START openInputConnection() ////////");
	  if (this._input != null)
		  return true;
	  try
	  {
		  ...
		  //this._input = new BufferedReader(new OutputStream(this._httpConnection.getOutputStream()));
	  ...
	  try
	  {
		  String str = this._input.readLine();
		  //process(this._input);
		  //System.out.println("SmsInterface : openInputConnection() : this._input.readLine() = " + this._input.readLine());
		  if (str == null) {
  ...
  public boolean connect(String paramString1, String paramString2, boolean paramBoolean)
  {
    ...
    for (int i = 0; (i < this._serverList.size()) && 
      (!openServerConnection((String)this._serverList.get(i))); i++);
    if (this._httpConnection == null) {
      return false;
    }
    try
  ...
  public SmsReplyList checkReplies()
  {
    ...
    try
    {
      ...
      while (((str = this._input.readLine()) != null) && (!str.startsWith("."))){
    	  System.out.println("SmsInterface : checkReplies : str = " + str);
  ...
  public void process(BufferedReader is) {
	    try {
	    	String inputLine;
	    	boolean stop = false;
	    	while (((inputLine = is.readLine()) != null) || (inputLine == "")) {
    			System.out.println("SmsInterface : process : " + is + " inputLine = " + inputLine);
	    	}
	    	is.close();
	    } catch (IOException e) {
